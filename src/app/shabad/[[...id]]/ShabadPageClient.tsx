...
// ============================================================================
// SHABAD VIEWER PAGE (Client Component)
// ============================================================================
...
export default function ShabadPageClient() {
  const params = useParams();
  // Support both Next.js params and direct pathname parsing (for static export in Capacitor)
  const idParam = params?.id;
  let shabadId: number;
  if (idParam) {
    shabadId = parseInt(Array.isArray(idParam) ? idParam[0] : (idParam as string), 10);
  } else if (typeof window !== 'undefined') {
    const match = window.location.pathname.match(/\/shabad\/(\d+)/);
    shabadId = match ? parseInt(match[1], 10) : NaN;
  } else {
    shabadId = NaN;
  }
