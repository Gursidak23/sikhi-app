...
export function OfflineManager({ language = 'pa' }: { language?: string }) {
  ...
  const downloadPresets = [
    { label: isPunjabi ? 'ਜਪੁ ਜੀ ਸਾਹਿਬ (੧-੮)' : 'Japji Sahib (1-8)', start: 1, end: 8 },
    { label: isPunjabi ? 'ਸੁਖਮਨੀ ਸਾਹਿਬ (੨੬੨-੨੯੬)' : 'Sukhmani Sahib (262-296)', start: 262, end: 296 },
    { label: isPunjabi ? 'ਸ੍ਰੀ ਰਾਗੁ (੧੪-੯੩)' : 'Sri Raag (14-93)', start: 14, end: 93 },
    { label: isPunjabi ? 'ਸਾਰੇ ੧੪੩੦ ਅੰਗ' : 'All 1430 Angs', start: 1, end: 1430 },
  ];

  return (
    <div className="bg-white dark:bg-neutral-800 rounded-xl border border-amber-200 dark:border-amber-800 p-4 sm:p-6">
      <h3 className={cn('text-lg font-semibold text-amber-900 dark:text-amber-200 mb-4', isPunjabi && 'font-gurmukhi')}>
        {isPunjabi ? '📱 ਆਫ਼ਲਾਈਨ ਸਮੱਗਰੀ' : '📱 Offline Content'}
      </h3>
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="text-center p-3 bg-amber-50 dark:bg-amber-900/30 rounded-lg">
            ...
            <p className={cn('text-xs text-amber-600 dark:text-amber-500', isPunjabi && 'font-gurmukhi')}>
              {isPunjabi ? 'ਅੰਗ ਸੇਵ' : 'Angs Saved'}
            </p>
          </div>
          <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
            <p className="text-2xl font-bold text-neela-700 dark:text-blue-400">{stats.banisCached}</p>
            <p className={cn('text-xs text-neela-600 dark:text-blue-500', isPunjabi && 'font-gurmukhi')}>
              {isPunjabi ? 'ਬਾਣੀਆਂ' : 'Banis'}
            </p>
          </div>
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm text-amber-700 dark:text-amber-400 mb-1">
            <span className={isPunjabi ? 'font-gurmukhi' : ''}>
              {isPunjabi ? 'ਡਾਊਨਲੋਡ ਹੋ ਰਿਹਾ ਹੈ...' : 'Downloading...'}
            </span>
      ...
      <div className="space-y-2">
        <p className={cn('text-sm font-medium text-neutral-600 dark:text-neutral-400', isPunjabi && 'font-gurmukhi')}>
          {isPunjabi ? 'ਡਾਊਨਲੋਡ ਕਰੋ:' : 'Download for offline:'}
        </p>
      ...
      {stats && stats.angsCached > 0 && (
        ...
        >
          {isPunjabi ? '🗑️ ਸਾਰਾ ਆਫ਼ਲਾਈਨ ਡਾਟਾ ਮਿਟਾਓ' : '🗑️ Clear All Offline Data'}
        </button>
      )}
    </div>
  );
